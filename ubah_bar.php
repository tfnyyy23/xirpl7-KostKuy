...
if( isset($_POST["submit"])){
    ...
    mysqli_query($koneksi, "UPDATE pwtbar SET nama_kost='$nama_kost', alamat_kost='$alamat_kost', harga='$harga' where id_kost='$id'");

    if( mysqli_affected_rows($koneksi) > 0){
        echo "<script>alert('Data Berhasil Diubah');
        document.location.href = 'pwtbar.php';</script>";
    } else{
...
<body style="background: url(assets/img/17_-_Ornament_Pattern.jpg); background-size: cover; background-attachment: fixed;">
    ...
    <?php 
    $id = $_GET['id_kost'];
    $result = mysqli_query($koneksi, "select * from pwtbar where id_kost='$id'");
